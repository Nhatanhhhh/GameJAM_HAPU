    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (hit != null)
            {
                ...
                // --- GỌI HIỆU ỨNG NẢY NHẸ ---
                LeafPickupBounce bounce = hit.GetComponent<LeafPickupBounce>();
                if (bounce != null)
                {
                    bounce.PlayBounce();
                    AudioManager.Instance?.PlaySFX("PickLeaf");
                ...
                {
                    Debug.Log($"{hit.name} không có Rigidbody2D — không thể kéo!");
                }
            }
        }
